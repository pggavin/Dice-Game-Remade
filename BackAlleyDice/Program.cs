...
@"            ██     ██████████     ██████████     ██████████     ██
            ...
            var sum = userRoll[0] + userRoll[1] + userRoll[2];
            // sum cached for ease of access

            // v USED FOR ALL TRIPLES/DOUBLES v
            var chosenNumber = -1;

            if ((userBetType >= 11) && userBetType < 17)
            {
                chosenNumber = userBetType - 10;
            }
            else if ((userBetType >= 5) && userBetType <= 10)
            {
                chosenNumber = userBetType - 4;
            }
            // Determines the chosen number for bets requiring triple or double nums
            // Uses the start points of all triple/double bets as an offset to quickly get their nums

            if ((userRoll[0] == userRoll[1]) && (userRoll[1] == userRoll[2]))
            {
                // if all numbers match
                if ((userRoll[0] == chosenNumber) && (userBetType >= 5 && userBetType < 11))
                {
                    betWon = true;
                }
                // SPECIFIC TRIPLE

                else if (userBetType == 17)
                {
                    betWon = true;
                }
                // ANY TRIPLE
            ...
            {
                if (sum >= 11 && sum <= 17 && userBetType == 1)
                {
                    betWon = true;
                }
                // BIG

                else if (sum >= 4 && sum <= 10 && userBetType == 2)
                {
                    betWon = true;
                }
                // SMALL
                ...
                if (userBetType >= 11 && userBetType < 17 && (
                   ((userRoll[0] == userRoll[1]) && chosenNumber == userRoll[0]) || // 0 & 1 match
                   ((userRoll[1] == userRoll[2]) && chosenNumber == userRoll[1]) || // 1 & 2 match
                   ((userRoll[2] == userRoll[0]) && chosenNumber == userRoll[2])))  // 0 & 2 match
                {
                    betWon = true;
                }
                // DOUBLES

                if ((sum % 2 == 1) && userBetType == 3)
                {
                    betWon = true;
                }
                // ODD

                else if ((sum % 2 == 0) && userBetType == 4)
                {
                    betWon = true;
                }
                // EVEN
            ...
            if (userBetType >= 18)
            {
                var range = userBetType - 17;
                // betType is used as an offset to know which vals to look for

                if (sum == 3 + range || sum == 18 - range)
                // looks for a match on either side
                {
                    betWon = true;
                }
                // 4 or 7, 5 or 16, 6 or 15, etc.
            }
            ...
            if (userBalance > 0 && userBalance < 100000)
            {
                goto GameLoop;
            }
